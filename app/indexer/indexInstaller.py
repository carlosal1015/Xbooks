...
def addToParentIndex(des, tipe, Xrc):
    ...
    band = "<!-- $BAND$ -->"
    new_band = "<tr onclick=\"window.location.replace('$LINK$'); updateExplorer_IFrame('$LINK$');\" style=\"background-color: rgb(55, 57, 58); width: 100vw; box-shadow: gray 2px 2px 2px;\"><th scope='row' style=\"border: none; width: 60vw;\">$XTITLE$</th><td style=\"border: none; width: 40vw;\">$TIMESTAMP$</td></tr>" + "\n\t" + band 

    card = "<!-- $CARD$ -->"
    start = "<td><div onclick=\"window.location.replace('$LINK$'); updateExplorer_IFrame(\'$LINK$\');\" class=\"card bg-light mb-3\" style=\"max-width: 20rem; background-color: rgba(39, 39, 39, 0.819) !important; color: rgb(200, 192, 188) !important; border: none; box-shadow: gray 5px 5px 5px;\"><div class=\"card-header\" style=\"background-color: rgba(37, 37, 37, 0.877) !important; border: none; color: white;\">$XTITLE$</div>"
    ...
    with open(index, 'w') as f:
        ...
        if tipe == "Xpage":
            body = BeautifulSoup(body.replace(band, new_band.replace('$XTITLE$', title).replace('$TIMESTAMP$', datetime.datetime.fromtimestamp(time.time()).strftime("%H:%M.%S|$MONTH$ %d %Y by Xbooks[bot]").replace("$MONTH$", chooseMonth(datetime.datetime.fromtimestamp(time.time()).strftime("%m")))).replace('$LINK$', '\\\\' + Xrc["gh_repo_name"] + '/' + notebook)), 'html.parser').prettify()
        if tipe == "Xbook":
            notebook = notebook + "/index.html"
            if os.path.exists("Xblog/" + des+ "/card.png"):
                body = BeautifulSoup(body.replace(card, new_card_img.replace('$XTITLE$', title).replace('$LINK$', '\\\\' + Xrc["gh_repo_name"] + '/' + notebook).replace("$CARD_IMG_SRC$", "card.png")), 'html.parser').prettify()
            else:
                body = BeautifulSoup(body.replace(card, new_card_noimg.replace('$XTITLE$', title).replace('$LINK$', '\\\\' + Xrc["gh_repo_name"] + '/' + notebook)), 'html.parser').prettify()
        f.write(body)
        f.close()
