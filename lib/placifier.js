...
function pdf_js(Xrc){
    let des = path.join(path.resolve(), "docs", "Assets", "js", "tools", "pdf.js");
    fs.readFile(
        des,
        (err, data)=>{
            if(err) process.exit(ccc.fail(err))
                ...
                fs.writeFile(
                    ...
                    .replace(/\$GH\$/g, Xrc["gh_repo_namespace"]),
                    err=>{
                        if(err) process.exit(ccc.fail(err))
...
function template_tplx(Xrc){
    let des = path.join(path.resolve("template.tplx"))
    fs.readFile(
        des,
        (err, data) => {
            if(err) process.exit(ccc.fail(err))
...
function env_yml(Xrc){
    let des = path.join(path.resolve("environment.yml"))
    fs.readFile(
        des,
        (err, data) => {
            if(err) process.exit(ccc.fail(err))
        ...
        (err, data) => {
            if (err) process.exit(ccc.fail(err))
                ...
                fs.writeFile(
                    ...
                    .replace(/\$BLOG_DESCRIPTION\$/g, Xrc["blog_description"])
                    .replace("$GH$", Xrc["gh_repo_namespace"])
