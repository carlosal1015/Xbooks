    ...
    initiate: () => {
        if(! fs.existsSync(rcfile_name)){
            ...
            inquirer.prompt(questions).then(answers => {
                repo = `https://github.com/${Xrc["GitHub_Username"]}/${Xrc["gh_repo_name"]}.git`;
                        ...
                        if(!isrepo){
                            git.init(err=>{
                                if(err) process.exit(ccc.fail(err))
                                    ...
                                    git.addRemote("origin", `https://github.com/${repo}.git`, err=>{
                                        if(err) process.exit(ccc.fail(err))
                                        else ccc.success(`adding remote https://github.com/${repo}.git as origin`)
                                    });
                                }
                            })
                        }
