...
const Xbooks_PATH = getInstalledPathSync('@xsoft/xbooks').toString();
...
var questions = [
    ...
    },
    {
        type: 'input',
        name: 'Pen_name',
        message: "Author's pen name (to display on your blog):"
    },
    {
        type: 'input',
        name: 'Email',
    ...
    },
    {
        ...
        message: "Author's GitHub.com username (to authenticate with GitHub.com):",
        validate: (value)=>{
            let pass = value.match(/^(?!\s*$).+/);
            if(pass) return true
            else return "this question is mandatory!"
    ...
    },
    {
        ...
        {
        type: 'input',
        name: 'gh_repo_namespace',
        message: "blog's GitHub repo namespace (under which this repo would belong):",
        validate: (value)=>{
            let pass = value.match(/^(?!\s*$).+/);
            if(pass) return true
            else return "this question is mandatory!"
    ...
    },
    {
        ...
        choices: ()=>{
            let list = [];
            list = getlist(path.join(Xbooks_PATH, 'src', 'CI', 'CI.list'));
            list.push("other");
            return list;
        }
    },
    {
        ...
        choices: getlist(path.join(Xbooks_PATH, 'src', 'themes', 'themes.list'))
    },
    // {
    //     type: 'list',
    //     name: 'Xplugin',
