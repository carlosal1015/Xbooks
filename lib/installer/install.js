...
module.exports = {
    ...
    install: (src_root, des_root) => {
        if (!fs.existsSync('.Xbooksrc')) {
            ...
            copier.copy_file(path.resolve(src_root, "src", "etc", "ref.bib"), path.resolve())
            copier.copy_file(path.resolve(src_root, "src", "etc", "template.tplx"), path.resolve())
            copier.copy_file(path.resolve(src_root, "src", "etc", ".gitpod.yml"), path.resolve())
            copier.copy_file(path.resolve(src_root, "src", "etc", ".gitpod.dockerfile"), path.resolve())
            copier.copy_file(path.resolve(src_root, "src", "etc", "environment.yml"), path.resolve())
            switch (this.Xrc.CI) {
                case "GitLabCI":
